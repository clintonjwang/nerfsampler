...
import numpy as np
from nerfsampler.inn.support import BoundingBox, Sphere, Support
...
def get_discretizations_for_args(args) -> Discretization:
    ...
    c2f = util.is_model_nerfsampler(args)
    ret = {}
    if 'train discretization' in dl_args:
        ret['input'] = ret['output'] = generate_discretization(domain=domain,
                    sampler=get_sampler_from_args(dl_args['train discretization'], dl_args, c2f=c2f))

    elif 'input discretization' in dl_args:
        ret['input'] = generate_discretization(domain=domain,
                    sampler=get_sampler_from_args(dl_args['input discretization'], dl_args, c2f=c2f))
    ...
    if 'test discretization' in dl_args:
        ret['test_in'] = generate_discretization(domain=domain,
                    sampler=get_sampler_from_args(dl_args['test discretization'], dl_args, c2f=c2f))
...
def generate_discretization(domain: Support, sampler: Sampler, **kwargs) -> Discretization:
    ...
    if method == "grid":
        coords = meshgrid_coords(*sampler['dims'], domain=domain.bounds, c2f=sampler['c2f'])
        shape = sampler['dims']

    elif method == "grid_slice":
        coords = meshgrid_coords(*sampler['dims'], domain=domain.bounds, c2f=sampler['c2f']) # (N,d-1)
...
def meshgrid_coords(*dims, domain, c2f=True,
