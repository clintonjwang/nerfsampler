...
class Upsample(nn.Module):
    def __init__(self, scale, spacing=1e-3, mode='nearest', kernel=None,
        ...
        # if mode == 'nearest':
        #     self.register_buffer('shift', torch.tensor([s/2 for s in spacing], dtype=dtype))
        # elif mode == 'k-NN':
        #     self.register_buffer('shift', torch.tensor(spacing, dtype=dtype))
        # elif mode == 'bilinear':
        #     self.register_buffer('shift', torch.tensor([s/2 for s in spacing], dtype=dtype))
        #     self.kernel_size = [s*3 for s in spacing]
