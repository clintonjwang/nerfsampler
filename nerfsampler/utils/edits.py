...
def animate_density_fxn(field, orig_fxn, orig_coords, eps=.01):
    ...
    def new_fxn(positions: TensorType["bs":..., 3]) -> TensorType["bs":..., 1]:
        ...
        in_new_range = (positions > target_bbox[0]).min(dim=-1).values & (positions < target_bbox[1]).min(dim=-1).values
        in_old_range = (positions > orig_bbox[0]).min(dim=-1).values & (positions < orig_bbox[1]).min(dim=-1).values & ~in_new_range
...
def animate_field_density(self, orig_coords, eps=.01):
    orig_bbox = orig_coords.min(dim=0).values-eps, orig_coords.max(dim=0).values+eps
    cog = orig_coords.mean(dim=0)
...
def animate_rgb(self, orig_coords, eps=.01):
    # orig_bbox = orig_coords.min(dim=0).values-eps, orig_coords.max(dim=0).values+eps
    cog = orig_coords.mean(dim=0)
    def get_outputs(ray_samples: RaySamples, density_embedding=None):
        assert density_embedding is not None
