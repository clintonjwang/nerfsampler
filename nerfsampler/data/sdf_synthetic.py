...
def get_ball_scene_dataloader(n_scenes, n_balls, max_radius=.5):
    while True:
        ...
        yield generate_3d_scenes(B=n_scenes, L=L, radius_range=(.2,max_radius), floor=False)

def generate_3d_scenes(B, L, radius_range, floor=False):
    ...
    # floor_color = torch.rand(3, device='cuda')
    # floor_coord = torch.rand(1, device='cuda')
