...
def train_segmenter(args: dict) -> None:
    ...
    global_step = 0
    trainsegdist = torch.load(DS_DIR+'/nerfsampler/cityscapes/trainsegdist.pt')
    ...
    grid_outputs = dl_args['discretization'] == 'grid' or not util.is_model_nerfsampler(args)
    for img_inr, segs in data_loader: # _, (B,n_cls,*dims)
        global_step += 1
        
        if util.is_model_nerfsampler(args): # DI-Net
...
def test_inr_segmenter(args):
    ...
    if dl_args['discretization type'] == 'grid':
        dims = dl_args['image shape']
    grid_outputs = dl_args['discretization type'] == 'grid' or not util.is_model_nerfsampler(orig_args)
    ...
    with torch.no_grad():
        for img_inr, segs in data_loader:
            ix += 1
            if util.is_model_nerfsampler(orig_args): # DI-Net
                logit_inr = model(img_inr) # (B,N,3)
                if grid_outputs:
                    logit_inr.sort()
                logits = logit_inr.values
...
def get_model_for_args(args):
    ...
    if hasattr(networks.nets.field2field, ntype):
        ...
        kwargs["sampler"] = point_set.get_sampler_from_args(args['data loading'])
        model = nerfsampler.baselines.nuft.NUFTSeg(depth=depth, C=C, **kwargs)
        return model.cuda()
    elif ntype.lower().startswith('hyper'):
        model = nerfsampler.baselines.hypernets.HypernetSeg(198915, depth=depth, C=C)
        return model.cuda()
    else:
    ...
    pretrained = args["network"]['pretrained']
