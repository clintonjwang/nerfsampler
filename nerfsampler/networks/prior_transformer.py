...
class PriorTransformer(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if self.positional_embedding is not None:
            hidden_states = hidden_states + self.positional_embedding.to(hidden_states.dtype)
...
@dataclass
class SimpleTransformerOutput(BaseOutput):
    """
    Args:
    ...
    transformed_states: torch.FloatTensor


class SimpleTransformer(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
        self,
        num_attention_heads: int = 32,
        ...
        self.time_proj = Timesteps(inner_dim, True, 0)
        self.time_embedding = TimestepEmbedding(inner_dim, inner_dim)
        self.proj_input = nn.Linear(embedding_dim, inner_dim)
        ...
        self.norm_out = nn.LayerNorm(inner_dim)
        self.proj_output = nn.Linear(inner_dim, embedding_dim)
    ...
    def forward(
        ...
        """
        Args:
            hidden_states (`torch.FloatTensor` of shape `(batch_size, num_embeddings, embedding_dim)`):
                Hidden states of the text embeddings the denoising process is conditioned on.
            timestep (`torch.long`):
                Current denoising step.
            proj_embedding (`torch.FloatTensor` of shape `(batch_size, embedding_dim)`):
        ...
        Returns:
            ...
            returning a tuple, the first element is the sample tensor.
        """
        batch_size, n_views, n_patches, _ = hidden_states.shape
        timesteps = timestep
        if not torch.is_tensor(timesteps):
            timesteps = torch.tensor([timesteps], dtype=torch.long, device=hidden_states.device)
        elif torch.is_tensor(timesteps) and len(timesteps.shape) == 0:
            timesteps = timesteps[None].to(hidden_states.device)
        timesteps = timesteps * torch.ones(batch_size, dtype=timesteps.dtype, device=timesteps.device)
        timesteps_projected = self.time_proj(timesteps).to(dtype=self.dtype)
        time_embeddings = self.time_embedding(timesteps_projected)

        hidden_states = self.proj_input(hidden_states)
        hidden_states = torch.cat(
            [
                hidden_states.reshape(batch_size, n_views*n_patches, -1),
                time_embeddings[:, None, :],
            ],
            dim=1,
        )
        ...
        hidden_states = self.norm_out(hidden_states)
        transformed_states = self.proj_output(hidden_states[:,:-1]).reshape(batch_size, n_views, n_patches, -1)

        if not return_dict:
            return (transformed_states,)

        return SimpleTransformerOutput(transformed_states=transformed_states)
